...
atv.config = { 
    ...
    "DEBUG_LEVEL": 4,
    "ROOT_URL": "http://trailers.apple.com/appletv/forecast.do", // "http://sample-web-server/trailers/sections/trailers-navigation.xml", //  
...
var atvutils = ATVUtils = {
    ...
    siteUnavailableError: function(index,url) {
        // TODO: localize
        return this.makeErrorDocument(url +" is currently unavailable. Try again later."+index, "Go to sample-xml.com/appletv for more information.");
    },
    ...
    loadURLInternal: function(url, method, headers, body, loader) {
        ...
        xhr = me.makeRequest(url, method, headers, body, function(xml) {
            try {
                ...
                loader(me.siteUnavailableError(1,url));
            }
        });
    },
            ...
            try {
                proxy.loadXML(xml, function(success) {
                    if ( !success ) {
                        ...
                        proxy.loadXML(me.siteUnavailableError(2,url));
                    }
                });
            } catch (e) {
                console.log("loadURL caught exception while loading " + url + ". " + e);
                proxy.loadXML(me.siteUnavailableError(3,url));
            }
        });
    },
            ...
            try {
                proxy.loadXML(xml, function(success) {
                    if ( success ) {
                        ...
                        proxy.loadXML(me.siteUnavailableError(4,url), function(success) {
                            if ( success ) {
                                atv.unloadPage();
                            }
                        });
                ...
                proxy.loadXML(me.siteUnavailableError(5,url), function(success) {
                    if ( success ) {
                        atv.unloadPage();
                    }
                });
