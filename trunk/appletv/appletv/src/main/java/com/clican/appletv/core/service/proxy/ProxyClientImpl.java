...
public class ProxyClientImpl extends BaseClient implements ProxyClient {
			...
			public void run() {
				try {
					...
					File fileDownload = new File(
							m3u8DownloadLine.getLocalPath() + ".download");
					...
					if (log.isDebugEnabled()) {
						log.debug("download m3u8 ts "
								+ new File(m3u8DownloadLine.getLocalPath())
										.getName());
					}
	...
	private void addAsyncM3u8TSRequest() {
		...
		if (log.isDebugEnabled()) {
			log.debug("downloading m3u8 ts "
					+ new File(m3u8DownloadLine.getLocalPath()).getName());
		}
		Runnable task = this.getM3u8Task(m3u8DownloadLine,
				this.m3u8Download.getM3u8Url());
		sendExecutor.submit(task);
	}
