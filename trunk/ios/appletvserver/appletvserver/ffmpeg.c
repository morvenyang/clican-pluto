...
static void sub2video_push_ref(InputStream *ist, int64_t pts) {
	...
	for (i = 0; i < ist->nb_filters; i++)
		av_buffersrc_add_ref(ist->filters[i]->filter,
				...
				AV_BUFFERSRC_FLAG_NO_CHECK_FORMAT | AV_BUFFERSRC_FLAG_NO_COPY
...
static void assert_codec_experimental(AVCodecContext *c, int encoder) {
	...
	if (c->codec->capabilities & CODEC_CAP_EXPERIMENTAL
		...
		codec = encoder ?
				avcodec_find_encoder(c->codec->id) :
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost) {
	...
	if ((avctx->codec_type == AVMEDIA_TYPE_AUDIO
		...
		int64_t max = ost->st->cur_dts
				+ !(s->oformat->flags & AVFMT_TS_NONSTRICT);
		if (ost->st->cur_dts && ost->st->cur_dts != AV_NOPTS_VALUE
				&& max > pkt->dts) {
	...
	while (bsfc) {
		...
		int a = av_bitstream_filter_filter(bsfc, avctx, NULL, &new_pkt.data,
				&new_pkt.size, pkt->data, pkt->size,
...
static void pre_process_video_frame(InputStream *ist, AVPicture *picture,
		void **bufp) {
	AVCodecContext *dec;
	AVPicture *picture2;
	...
	if (do_deinterlace) {
		...
		if (avpicture_deinterlace(picture2, picture, dec->pix_fmt, dec->width,
				dec->height) < 0) {
...
static void do_subtitle_out(AVFormatContext *s, OutputStream *ost,
		InputStream *ist, AVSubtitle *sub) {
	...
	for (i = 0; i < nb_frames; i++) {
		...
		if (s->oformat->flags & AVFMT_RAWPICTURE
			...
			if (ost->forced_kf_index < ost->forced_kf_count
					&& big_picture.pts
							>= ost->forced_kf_pts[ost->forced_kf_index]) {
			...
			if (got_packet) {
				if (pkt.pts == AV_NOPTS_VALUE
						&& !(enc->codec->capabilities & CODEC_CAP_DELAY))
				...
				if (pkt.pts != AV_NOPTS_VALUE)
					pkt.pts = av_rescale_q(pkt.pts, enc->time_base,
							ost->st->time_base);
				if (pkt.dts != AV_NOPTS_VALUE)
					pkt.dts = av_rescale_q(pkt.dts, enc->time_base,
							ost->st->time_base);
		...
		while (1) {
			...
			if (picref->pts != AV_NOPTS_VALUE) {
				filtered_frame->pts = frame_pts = av_rescale_q(picref->pts,
						...
						- av_rescale_q(of->start_time, AV_TIME_BASE_Q,
								ost->st->codec->time_base);
...
static void print_report(int is_last_report, int64_t timer_start,
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
			...
			snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
					"frame=%5d fps=%3.*f q=%3.1f ", frame_number, fps < 9.95,
					fps, q);
		...
		if ((is_last_report || !ost->finished)
				&& ost->st->pts.val != AV_NOPTS_VALUE)
	...
	snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "size=%8.0fkB time=",
			total_size / 1024.0);
	snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
	...
	if (is_last_report) {
		...
		av_log(NULL, AV_LOG_INFO,
				...
				video_size / 1024.0, audio_size / 1024.0,
				subtitle_size / 1024.0, extra_size / 1024.0,
...
static void flush_encoders(void) {
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		for (;;) {
			int (*encode)(AVCodecContext*, AVPacket*, const AVFrame*,
					int*) = NULL;
			const char *desc;
			int64_t *size;
...
static void do_streamcopy(InputStream *ist, OutputStream *ost,
		const AVPacket *pkt) {
	OutputFile *of = output_files[ost->file_index];
	int64_t ost_tb_start_time = av_rescale_q(of->start_time, AV_TIME_BASE_Q,
			ost->st->time_base);
	AVPicture pict;
	AVPacket opkt;
	...
	if (ost->st->codec->codec_type == AVMEDIA_TYPE_VIDEO
		...
		avpicture_fill(&pict, opkt.data, ost->st->codec->pix_fmt,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output) {
	...
	if (resample_changed) {
		...
		av_get_channel_layout_string(layout1, sizeof(layout1),
				ist->resample_channels, ist->resample_channel_layout);
		av_get_channel_layout_string(layout2, sizeof(layout2), avctx->channels,
				decoded_frame->channel_layout);

		av_log(NULL, AV_LOG_INFO,
				"Input stream #%d:%d frame changed from rate:%d fmt:%s ch:%d chl:%s to rate:%d fmt:%s ch:%d chl:%s\n",
				ist->file_index, ist->st->index, ist->resample_sample_rate,
				av_get_sample_fmt_name(ist->resample_sample_fmt),
				ist->resample_channels, layout1, decoded_frame->sample_rate,
				av_get_sample_fmt_name(decoded_frame->format), avctx->channels,
				layout2);
	...
	if (decoded_frame->pts != AV_NOPTS_VALUE) {
		ist->dts = ist->next_dts = ist->pts = ist->next_pts = av_rescale_q(
				decoded_frame->pts, avctx->time_base, AV_TIME_BASE_Q);
	...
	if (decoded_frame->pts != AV_NOPTS_VALUE)
		decoded_frame->pts = av_rescale_q(decoded_frame->pts, decoded_frame_tb,
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output) {
	...
	if (best_effort_timestamp != AV_NOPTS_VALUE)
		ist->next_pts = ist->pts = av_rescale_q(decoded_frame->pts =
				best_effort_timestamp, ist->st->time_base, AV_TIME_BASE_Q);
...
av_log	(NULL, AV_LOG_INFO, "decoder -> ist_index:%d type:video "
	...
	if (resample_changed) {
		av_log(NULL, AV_LOG_INFO,
				...
				ist->file_index, ist->st->index, ist->resample_width,
				ist->resample_height,
	...
	frame_sample_aspect = av_opt_ptr(avcodec_get_frame_class(), decoded_frame,
			"sample_aspect_ratio");
	for (i = 0; i < ist->nb_filters; i++) {
		int changed = ist->st->codec->width
				!= ist->filters[i]->filter->outputs[0]->w
				|| ist->st->codec->height
						!= ist->filters[i]->filter->outputs[0]->h
				|| ist->st->codec->pix_fmt
						!= ist->filters[i]->filter->outputs[0]->format;
		...
		if (ist->dr1 && decoded_frame->type == FF_BUFFER_TYPE_USER
			...
			AVFilterBufferRef *fb = avfilter_get_video_buffer_ref_from_arrays(
					...
					AV_PERM_READ | AV_PERM_PRESERVE, ist->st->codec->width,
			...
			av_buffersrc_add_ref(ist->filters[i]->filter, fb,
					AV_BUFFERSRC_FLAG_NO_CHECK_FORMAT
							...
							| AV_BUFFERSRC_FLAG_PUSH);
		} else if (av_buffersrc_add_frame(ist->filters[i]->filter,
				decoded_frame, AV_BUFFERSRC_FLAG_PUSH) < 0) {
			av_log(NULL, AV_LOG_FATAL,
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output) {
	AVSubtitle subtitle;
	int i, ret = avcodec_decode_subtitle2(ist->st->codec, &subtitle, got_output,
			pkt);
	if (ret < 0 || !*got_output) {
		if (!pkt->size)
			sub2video_flush(ist);
	...
	if (!ist->saw_first_ts) {
		ist->dts =
				ist->st->avg_frame_rate.num ?
						-ist->st->codec->has_b_frames * AV_TIME_BASE
	...
	if (!ist->decoding_needed) {
		...
		case AVMEDIA_TYPE_AUDIO:
			ist->next_dts += ((int64_t) AV_TIME_BASE
					* ist->st->codec->frame_size) / ist->st->codec->sample_rate;
			break;
		case AVMEDIA_TYPE_VIDEO:
			if (pkt->duration) {
	...
	if (ist->decoding_needed) {
		...
		if (!codec) {
			snprintf(error, error_len,
					...
					avcodec_get_name(ist->st->codec->codec_id), ist->file_index,
					ist->st->index);
...
static int transcode_init(void) {
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (ost->stream_copy) {
			...
			extra_size = (uint64_t) icodec->extradata_size
			...
			if (!strcmp(oc->oformat->name, "avi")) {
				if (copy_tb < 0
						...
						&& 0.5 / av_q2d(ist->st->r_frame_rate)
								> av_q2d(icodec->time_base)
						&& av_q2d(ist->st->time_base) < 1.0 / 500
						&& av_q2d(icodec->time_base) < 1.0 / 500
						|| copy_tb == 2) {
			...
			case AVMEDIA_TYPE_AUDIO:
				...
				if ((codec->block_align == 1 || codec->block_align == 1152)
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (ost->encoding_needed) {
			...
			if (dec && dec->subtitle_header) {
				...
				ost->st->codec->subtitle_header = av_mallocz(
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (ost->sync_ist != input_streams[ost->source_index])
			av_log(NULL, AV_LOG_INFO, " [sync #%d:%d]",
					ost->sync_ist->file_index, ost->sync_ist->st->index);
		if (ost->stream_copy)
	...
	if (key == 'c' || key == 'C') {
		...
		if (k > 0
			...
			for (i = 0; i < nb_filtergraphs; i++) {
				...
				if (fg->graph) {
					if (time < 0) {
						ret = avfilter_graph_send_command(fg->graph, target,
								command, arg, buf, sizeof(buf),
	...
	if (key == 'd' || key == 'D') {
		...
		if (key == 'D') {
			...
			if (!debug)
				debug = 1;
			while (debug
					& (FF_DEBUG_DCT_COEFF | FF_DEBUG_VIS_QP
...
static int process_input(int file_index) {
	...
	if (!ist->wrap_correction_done
			&& input_files[file_index]->ctx->start_time != AV_NOPTS_VALUE
			&& ist->st->pts_wrap_bits < 64) {
		int64_t stime = av_rescale_q(input_files[file_index]->ctx->start_time,
