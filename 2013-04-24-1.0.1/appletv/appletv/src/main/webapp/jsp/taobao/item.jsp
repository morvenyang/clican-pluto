					...
					<sections>
						<shelfSection>
							<items>
								...
								<actionButton id="album_3" onSelect="atv.loadURL('${serverurl}/ctl/weibo/createStatus.xml?itemId=${item.numIid}&amp;deviceId='+atv.device.udid+'&amp;feature=10&amp;title='+encodeURIComponent('${fn:escapeXml(item.title)}')+'&amp;shareURL='+encodeURIComponent('${fn:escapeXml(item.detailUrl)}')+'&amp;imageURL=${fn:escapeXml(item.picUrl)}');" onPlay="atv.loadURL('${serverurl}/ctl/weibo/createStatus.xml?itemId=${item.numIid}&amp;deviceId='+atv.device.udid+'&amp;feature=10&amp;title='+encodeURIComponent('${fn:escapeXml(item.title)}')+'&amp;shareURL='+encodeURIComponent('${fn:escapeXml(item.detailUrl)}')+'&amp;imageURL=${fn:escapeXml(item.picUrl)}');">
									<title>分享</title>
									<image>${serverurl}/image/weibo/share.png</image>
								</actionButton>
								<actionButton id="album_4" onSelect="atv.loadURL('${serverurl}/ctl/taobao/itemSelect.xml?itemId=${item.numIid}&amp;sellerId=${sellerId}');" onPlay="atv.loadURL('${serverurl}/ctl/taobao/itemSelect.xml?itemId=${item.numIid}&amp;sellerId=${sellerId};">
									<title>购买</title>
									<image>${serverurl}/image/weibo/share.png</image>
								</actionButton>
