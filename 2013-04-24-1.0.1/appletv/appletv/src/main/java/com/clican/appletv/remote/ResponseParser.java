...
package com.clican.appletv.remote;
...
public class ResponseParser {
   ...
   public static int performSearch(byte[] raw,  Pattern listenFor, boolean haltmlit)
      ...
      final int hits = ResponseParser.search(stream,  listenFor, stream.available(), haltmlit);
      return hits;
   }

   public static Response performParse(byte[] raw,  Pattern listenFor) throws IOException {
      final DataInputStream stream = new DataInputStream(new ByteArrayInputStream(raw));
      final Response resp = ResponseParser.parse(stream,  listenFor, stream.available());
      return resp;
   }

   public static Response performParse(byte[] raw) throws IOException {
      final DataInputStream stream = new DataInputStream(new ByteArrayInputStream(raw));
      return ResponseParser.parse(stream,  null, stream.available());
   }
      ...
      while (handle > 0) {
         ...
         if (haltmlit && key.equals("mlit")) {
            ...
            if (listenFor.matcher(key).matches()) {
               ...
               hits += ResponseParser.search(raw,  listenFor, length, haltmlit);
            }
   ...
   private static Response parse(DataInputStream raw, Pattern listenFor, int handle)
            throws IOException {
      final Response resp = new Response();
      int progress = 0;
      ...
      while (handle > 0) {
         ...
         if (BRANCHES.matcher(key).matches()) {
            ...
            final Response branch = ResponseParser.parse(raw,  listenFor, length);
