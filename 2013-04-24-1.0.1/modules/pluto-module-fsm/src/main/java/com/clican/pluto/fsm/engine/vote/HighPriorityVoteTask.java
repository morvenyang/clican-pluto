...
public class HighPriorityVoteTask extends TaskStateImpl {
	...
	protected List<Task> assignTasks(State state, Event event) {
		...
		taskDao.setVariable(firstPriorityTask,
				Parameters.FIRST_PRIORITY_ASSIGNEE.getParameter(), true);
		return taskList;
	}
	...
	protected void handleTask(Task task, Event event) {
		...
		VoteResult voteResult = VoteResult.convert((String) this
				.getVariableValue(Parameters.VOTE_RESULT.getParameter(), event,
						false));
		...
		if (eventType == EventType.TASK) {
			...
			Boolean firstPriorityAssignee = (Boolean) this.getVariableValue(
					Parameters.FIRST_PRIORITY_ASSIGNEE.getParameter(), task,
					false);
			if (firstPriorityAssignee != null && firstPriorityAssignee) {
				if (log.isDebugEnabled()) {
					log.debug("First priority assignee is voted,voteResult["
							+ voteResult + "]");
				}
				List<Task> notCompletedTaskList = taskDao
						.findActiveTasksByState(event.getState());
				for (Task notCompletedTask : notCompletedTaskList) {
					...
					taskDao.setVariable(notCompletedTask,
							Parameters.VOTE_RESULT.getParameter(),
							VoteResult.DISCLAIM.getVoteResult());
					taskDao.save(notCompletedTask);
				}
			} else {
				VoteResult lastVoteResult = VoteResult.convert((String) this
						.getVariableValue(
								Parameters.VOTE_RESULT.getParameter(), event
										.getState(), false));
				if (lastVoteResult != null) {
					List<Task> completedTaskList = taskDao
							.findActiveTasksByState(event.getState());
			...
			taskDao.setVariable(task, Parameters.VOTE_RESULT.getParameter(),
		...
		if (resetLastVoteResult) {
			stateDao.setVariable(event.getState(), Parameters.VOTE_RESULT
					.getParameter(), voteResult.getVoteResult());
			recordLastVoteResultAndSuggestion(task.getState().getSession()
					.getId(), voteResult.getVoteResult(), (String) this
					.getVariableValue(Parameters.AUDIT_SUGGESTION
							.getParameter(), event, false), task.getAssignee());
		}
