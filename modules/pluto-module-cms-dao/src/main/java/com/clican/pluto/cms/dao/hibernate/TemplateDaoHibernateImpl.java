...
public class TemplateDaoHibernateImpl extends BaseDao implements TemplateDao {
	...
	public List<ITemplate> getSelectedTemplates(IDataModel dataModel) {
		String modelName = dataModel.getClass().getAnnotation(Entity.class)
				.name();
		...
		sql.append(modelName);
		sql
				.append("Relation r where r.template=t and r.dataModel=m and m.id = :id");
		return getHibernateTemplate().findByNamedParam(sql.toString(), "id",
				dataModel.getId());
	}
	...
	public List<ITemplateModelRelation> getTemplateModelRelations(
		...
		sql.append(modelName);
		sql
				.append("Relation r where r.template=t and r.dataModel=m and m.id in ");
	...
	public List<ITemplateModelRelation> getTemplateModelRelations(
		...
		sql.append(modelName);
		sql
				.append("Relation r where r.template=t and r.dataModel=m and m.parent.path like :pathExpression");
	...
	public int getTemplateModelRelationCount(ModelDescription modelDescription,
		...
		sql
				.append("Relation r where r.template=t and r.dataModel=m and m.parent.path like :pathExpression");
		return (Integer) getHibernateTemplate().findByNamedParam(
				sql.toString(), "pathExpression", "pathExpression").get(0);
	}
