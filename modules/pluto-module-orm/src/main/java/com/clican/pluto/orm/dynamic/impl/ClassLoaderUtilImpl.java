...
public class ClassLoaderUtilImpl implements ClassLoaderUtil {
	...
	public void configureTemplateDirectorySiteRelations(IDataModel dataModel,
		...
		String name = Constants.DYNAMIC_MODEL_PACKAGE + "."
				+ Constants.DEFAULT_TEMPLATE_CLASS_NAME + entityName
				+ Constants.DEFAULT_SITE_CLASS_NAME + "Relation";
		try {
	...
	public ISite newSite() {
		ISite site = null;
		try {
			site = (ISite) (dynamicClassLoader
					.loadClass(Constants.DYNAMIC_MODEL_PACKAGE + ".Site")
					.newInstance());
		} catch (Exception e) {
			throw new PlutoException("The " + Constants.DYNAMIC_MODEL_PACKAGE
					+ ".Site" + " can't be found in the classloader.", e);
		}
		return site;
	}
